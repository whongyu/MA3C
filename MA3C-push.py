    ...
    for i, agent in enumerate(agents):
        for j, _agent in enumerate(agents):
            if i == j or adjacent_matrix[i, j] > 0.0:
                y_list[i].append(_agent.y * adjacent_matrix[i, j])
                reward_list[i].append(np.array(_agent.reward) * adjacent_matrix[i, j])
                tao = []
                for name, param in _agent.tao.named_parameters():
                    tao.append(deepcopy(param.cpu().data).numpy().reshape(1, -1).squeeze(0))
                tao_list[i].append(np.array(tao) * adjacent_matrix[i, j])
    consensus_y = [np.sum(np.asarray(x), axis=0) for x in y_list]
    consensus_tao = []
    for i in range(N_agent):
        tao = tao_list[i]
        temp = [[] for i in range(len(tao[0]))]
        for j in range(len(tao)):
            for k in range(len(tao[0])):
                temp[k].append(tao[j][k])
        consensus_tao.append(np.sum(temp, axis=1))
        for k in range(len(consensus_tao[i])):
            consensus_tao[i][k] = consensus_tao[i][k].reshape(agents[0].shape[k])
    consensus_reward = [np.sum(np.asarray(x), axis=0) for x in reward_list]
...
def learn(agent_list, states_var, next_states_var, rewards_var, actions_var, rewards, adjacent_matrix):
    ...
    for j in range(N_agent):
        ...
        for name, param in agent_list[j].value_network.named_parameters():
            param.data = deepcopy(temp[i]) / consensus_y[j]
            i += 1
